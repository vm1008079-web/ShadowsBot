...
const handler = async (m, { conn, args, __dirname, usedPrefix, command }) => {
  try {
    ...
    if (!/audio/.test(mime)) throw `🎧 Responde a un audio para aplicar efecto.\nEj: *${usedPrefix + command}*`
    ...
    throw typeof e === 'string' ? e : '⚠️ Ocurrió un error procesando el audio.'
  }
}

handler.help = ['bass', 'blown', 'deep', 'earrape', 'fast', 'fat', 'nightcore', 'reverse', 'robot', 'slow', 'smooth', 'tupai'].map(v => v + ' [vn]')
handler.tags = ['tools']
handler.command = /^(bass|blown|deep|earrape|fas?t|fat|nightcore|reverse|robot|slow|smooth|tupai|squirrel|chipmunk)$/i
